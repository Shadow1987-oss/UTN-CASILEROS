    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ optional($sancion->usuario)->nombre ?? $sancion->idusuario }}</td>
                <td>{{ $sancion->sancion }}</td>
                <td>{{ $sancion->motivo }}</td>
                <td class="actions">
                    <a class="btn secondary" href="{{ route('sanciones.edit', $sancion) }}">Editar</a>
                    <form method="POST" action="{{ route('sanciones.destroy', $sancion) }}" onsubmit="return confirm('¿Eliminar esta sanción?');" style="display:inline;">
                        @csrf
                        @method('DELETE')
            ...
            @empty
            <tr><td colspan="5" class="muted">Sin sanciones aún.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
