    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ optional($r->student)->nombre ?? $r->matricula }}</td>
                <td class="actions">
                    <form method="POST" action="{{ route('recibe.destroy', $r) }}" onsubmit="return confirm('¿Eliminar este recibo?');" style="display:inline;">
                        @csrf
                        @method('DELETE')
            ...
            @empty
            <tr><td colspan="4" class="muted">Sin recibos aún.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
